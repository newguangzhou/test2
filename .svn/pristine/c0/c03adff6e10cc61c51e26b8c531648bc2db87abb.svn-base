...
class HelperHandler(xmq_web_handler.XMQWebHandler):
    ...
    def check_account_exist(self, logprefix, res, auth_type, auth_id):
        ...
        if phone_num is None:
            logging.warning("%s, account not exist!, %s", logprefix, self.dump_req())
    ...
    @gen.coroutine
    @asynchronous
    def check_account_exist_by_phone_num(self, logprefix, res, auth_type, phone_num):
        auth_dao = self.settings["auth_dao"]
        
        # 检查账号是否已经存在
        auth_id = yield auth_dao.has_auth_info_by_mobile_num(auth_type, phone_num)
        if auth_id is None:
            logging.warning("%s, account not exist!, %s", logprefix, self.dump_req())
            res["status"] = error_codes.EC_USER_NOT_EXIST
            self.res_and_fini(res)
    ...
    def check_account_status(self, logprefix, res, auth_type, auth_id):
        ...
        if ec != error_codes.EC_SUCCESS:
            logging.warning("%s, check account status failed, ec=%u extra=%s", logprefix, ec, str(extra))
            res["status"] = ec
    ...
    def check_token(self, logprefix, res, auth_type, auth_id, token):
        ...
        if ec != error_codes.EC_SUCCESS:
            logging.warning("%s, check token failed, ec=%u %s", logprefix, ec, self.dump_req())
            res["status"] = ec
            self.res_and_fini(res) 
            raise gen.Return(False)
        raise gen.Return(True)
